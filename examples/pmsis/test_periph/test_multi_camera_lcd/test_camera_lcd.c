...
void test_camera_with_lcd(void)
{
    ...
    if (errors)
    {
        printf("Failed to open camera0 : %ld\n", errors);
        pmsis_exit(-2);
    }

    // opening camera 1
    errors = open_camera(&cam1);
    if (errors)
    {
        printf("Failed to open camera1 : %ld\n", errors);
        pmsis_exit(-2);
    }
    ...
    while (1)
    {
        ...
        printf("Image captured camera 0.\n");
        
        // camera 1
        pi_camera_capture(&cam1, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT);
        printf("Image captured camera 1.\n");
