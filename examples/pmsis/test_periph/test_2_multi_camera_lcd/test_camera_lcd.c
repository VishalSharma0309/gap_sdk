...
static void cam_handler(void *arg)
{
    ...
    PRINTF("Cam0: image captured\n");
   
    // to either display on the lcd (if possible) or write the image file
    ...
    #else
    sprintf(imgName, "../../../img0_OUT_%ld.ppm", idx);
    ...
    pi_display_write_async(&lcd, &buffer, 0, 0, LCD_WIDTH, LCD_HEIGHT, &task);
    #else
    sprintf(imgName, "../../../img1_OUT_%ld.ppm", idx);
    WriteImageToFile(imgName, CAMERA_WIDTH, CAMERA_HEIGHT, sizeof(uint8_t), imgBuff0, GRAY_SCALE_IO);
    idx++;
    //lcd_handler(NULL);
    #endif  /* DISPLAY */
...
// lcd_handler for camera 1
static void lcd_handler1(void *arg)
{
    pi_task_callback(&task, cam_handler, NULL);

    pi_camera_capture_async(&cam1, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT, &task);
    pi_camera_control(&cam1, PI_CAMERA_CMD_START, 0);
}
#if defined(DISPLAY)
...
void test_camera_with_lcd(void)
{
    ...
    if (errors)
    {
        printf("Failed to open camera 0: %ld\n", errors);
        pmsis_exit(-2);
    }

    // for cam 1
    errors1 = open_camera(&cam1);
    if (errors1)
    {
        printf("Failed to open camera 1: %ld\n", errors1);
        pmsis_exit(-2);
    }
    ...
    while (1)
    {
        ...
        pi_task_callback(&task, cam_handler, NULL);
        pi_camera_capture_async(&cam, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT, &task);
        printf("Image capture cam 0.\n");
        pi_camera_control(&cam, PI_CAMERA_CMD_START, 0);
        printf("Camera 0 start.\n");
        pi_task_wait_on(&task);
        #else
        printf("Camera 0 start.\n");
        pi_camera_control(&cam, PI_CAMERA_CMD_START, 0);
        pi_camera_capture(&cam, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT);
        printf("Image captured cam 0.\n");
        pi_camera_control(&cam, PI_CAMERA_CMD_STOP, 0);
        printf("Camera 0 stop.\n");
        #if defined(DISPLAY)
        ...
        pi_task_callback(&task, cam_handler, NULL);
        pi_camera_capture_async(&cam1, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT, &task);
        printf("Image capture cam 1.\n");
        pi_camera_control(&cam1, PI_CAMERA_CMD_START, 0);
        printf("Camera 1 start.\n");
        pi_task_wait_on(&task);
        #else
        printf("Camera 1 start.\n");
        pi_camera_control(&cam1, PI_CAMERA_CMD_START, 0);
        pi_camera_capture(&cam1, imgBuff0, CAMERA_WIDTH * CAMERA_HEIGHT);
        printf("Image captured cam 1.\n");
        pi_camera_control(&cam1, PI_CAMERA_CMD_STOP, 0);
        printf("Camera 1 stop.\n");
        #if defined(DISPLAY)
        pi_display_write(&lcd, &buffer, 0, 0, LCD_WIDTH, LCD_HEIGHT);
