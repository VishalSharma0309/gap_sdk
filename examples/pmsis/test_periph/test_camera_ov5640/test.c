...
static void ov5640_set_qvga (struct pi_device *device)
{

    uint8_t reg_value=0x4F;
    /*
    for(int i=0; i<(sizeof(OV5640_QSXGA2QVGA)/sizeof(ov5640_reg_t)); i++)
        pi_camera_reg_set(device, OV5640_QSXGA2QVGA[i].addr, &(OV5640_QSXGA2QVGA[i].value));
        */
    for(int i=0; i<(sizeof(OV5640_RGB_QVGA)/sizeof(ov5640_reg_t)); i++)
        pi_camera_reg_set(device, OV5640_RGB_QVGA[i].addr, &(OV5640_RGB_QVGA[i].value));

#ifdef DEBUG
    for(int i=0; i<(sizeof(OV5640_RGB_QVGA)/sizeof(ov5640_reg_t)); i++)
    {
        pi_camera_reg_get(device, OV5640_RGB_QVGA[i].addr, &reg_value);
        printf("addr %X = %X\n", OV5640_RGB_QVGA[i].addr, reg_value);
    }
#endif
}
